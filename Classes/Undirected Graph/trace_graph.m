...
%    .S: [k x 1 cell] edge paths in the image     
    ...
    for i=1:n % for each feature
       ...
       for k=1:nb
          ...
          if ~Z.U(lind(br,rowOfT)) % If this branch isn't yet explained
             path = [start; br];
             if Z.J(lind(br,rowOfT))
                % criteria so each junction-to-junction is added once
                if lind(br,rowOfT) > lind(start,rowOfT), continue; end
             else
                Z.U(lind(br,rowOfT)) = true;
                tabu = false(size(Z.T));
                tabu(lind(blist,rowOfT)) = true; % tabu list, of all
                    % other branch points from that junction
                tabu(lind(start,rowOfT)) = true;
...
function y = lind(pts,row)

% Make linear index
%
% Input
% pts: [n x 2] points to convert (Rows)
% row: [1 x 1 (int)] row of matrix
y = pts(:,1) + ((pts(:,2)-1).*row);
end
...
function [path,Z] = continuepath(path,Z,tabu)    
    next = path(end,:);
    rowOfT = size(Z.T,1);
    while ~Z.J(lind(next,rowOfT))
        next = pathstep(next,Z,tabu);
        Z.U(lind(next,rowOfT)) = true;
        path = [path; next];
